<sch:schema queryBinding="xslt2" xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:lichaam//tekst:Nummer">
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:RegelingMutatie">
      ...
      <sch:p>De identificerende waarden van de attributen @was en @FRBRwork MOET onderling
        consistent zijn.</sch:p>
      <sch:assert role="fatal" test="starts-with(@was, $workID)"> De waarde voor de @was van
        RegelingMutatie <sch:value-of select="@componentnaam"/> begint niet met de waarde van
        @FRBRwork. </sch:assert>
      <sch:p>De identificerende waarde van het attributen @wordt en @FRBRwork MOET onderling
        consistent zijn.</sch:p>
      <sch:assert role="fatal" test="starts-with(@wordt, $workID)"> De waarde voor de @wordt van
        RegelingMutatie <sch:value-of select="@componentnaam"/> begint niet beginnen met de waarde
        van @FRBRwork. </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern>
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:li/tekst:table">
      ...
      <sch:report role="warning"
        test="ancestor::tekst:NieuweRegeling or ancestor::tekst:MaakInitieleRegeling"> Een tabel in
        Lijstitem <sch:value-of select="parent::Li/@wId"/> is niet toegestaan voor nieuwe
        regelingen. </sch:report>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern>
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:Lid">
      ...
      <sch:assert role="error" test="tekst:LidNummer"> Het Lid <sch:value-of select="@wId"/> van
        Artikel <sch:value-of select="parent::tekst:Artikel/@wId"/> heeft geen LidNummer.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <!--  -->
  <sch:pattern>
    <sch:title>De referentie en de tekst van een ExtIoRef moeten gelijk zijn [D].</sch:title>
    <sch:rule context="tekst:ExtIoRef">
      ...
      <sch:assert test=". = $waarde"> De JOIN identifier van ExtIoRef <sch:value-of select="@wId"/>
        in de tekst is niet gelijk aan de referentie. </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern>
  ...
  <sch:pattern>
    ...
    <sch:rule context="//tekst:NieuweTekst | //tekst:VerwijderdeTekst">
      ...
      <sch:assert role="error" test="ancestor::tekst:RegelingMutatie"> Tekstuele wijziging is niet
        ...
        select="ancestor::tekst:*[@wId][1]/@wId"/>" buiten de context van een tekst:RegelingMutatie.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:table">
      <sch:assert role="error" test="tekst:tgroup/@cols >= 2"> De tabel met <sch:value-of
  ...
  <sch:pattern>
    ...
    <sch:rule context="tekst:entry[@namest ne @nameend]">
      ...
      <sch:assert role="error"
        ...
        select="local-name(ancestor::tekst:thead | ancestor::tekst:tbody)"/>, van de tabel met id:
        <sch:value-of select="ancestor::tekst:table/@wId"/>, kent een namest groter dan de nameend
      </sch:assert>
    </sch:rule>
    <sch:rule context="tekst:entry[@namest and @colname]">
      ...
      <sch:assert role="error" test="@namest ne @colname"> De entry met @namest <sch:value-of
        select="@namest"/>, van de <xsl:value-of select="count(preceding-sibling::tekst:row) + 1"/>e
        rij, van <sch:value-of select="local-name(ancestor::tekst:thead | ancestor::tekst:tbody)"/>,
        van de tabel met id: <sch:value-of select="ancestor::tekst:table/@wId"/>, kent een @namest
        die ongelijk is aan @colname. </sch:assert>
    </sch:rule>
    <sch:rule context="tekst:row">
      ...
      <sch:report role="error" test="count(tekst:entry) = count(tekst:entry[@morerows = '1'])"> Alle
        cellen van de <xsl:value-of select="count(preceding-sibling::tekst:row) + 1"/>e rij van
        tabel <sch:value-of select="ancestor::tekst:table/@wId"/> hebben een @morerows attribuut met
        de waarde '1'. </sch:report>
    </sch:rule>
    <sch:rule context="tekst:entry/@colname | tekst:entry/@namest | tekst:entry/@nameend">
      ...
      <sch:assert role="error" test="ancestor::tekst:tgroup/tekst:colspec[@colname = $id]"> De entry
        met @<sch:value-of select="name()"/> <sch:value-of select="."/> van de <xsl:value-of
        select="count(parent::tekst:entry/preceding-sibling::tekst:row) + 1"/>e rij, van
        <sch:value-of select="local-name(ancestor::tekst:thead | ancestor::tekst:tbody)"/>, van de
        tabel met id: <sch:value-of select="ancestor::tekst:table/@wId"/>, verwijst niet naar een
        bestaande kolom. </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern>
