<sch:schema queryBinding="xslt2" xmlns:data="https://standaarden.overheid.nl/stop/imop/data/"
  ...
  <sch:pattern id="sch_lvbba_006">
    <sch:title>Besluit met soort werk '/join/id/stop/work_019</sch:title>
    <sch:rule context="lvbba:AanleveringBesluit/lvbba:RegelingVersieInformatie">
      <sch:let name="soortWork" value="normalize-space(data:ExpressionIdentificatie/data:soortWork)"/>
      <sch:p>De identificatie van een regelingversie moet als soort werk '/join/id/stop/work_019'
        hebben</sch:p>
      <sch:assert id="BHKV1006" role="error" test="$soortWork = '/join/id/stop/work_019'">{"code": "BHKV1006", "id": "<sch:value-of select="$soortWork"/>", "melding": "Het geleverde regelingversie heeft als soortWork '<sch:value-of select="$soortWork"/>' , Dit moet zijn: '/join/id/stop/work_019'.", "ernst": "fout"},</sch:assert>
    </sch:rule>
  </sch:pattern>
  <!--  -->
