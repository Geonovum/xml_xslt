...
<sch:schema queryBinding="xslt2" xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <sch:pattern id="sch_tekst_022" see="tekst:Al">
    ...
    <sch:rule context="tekst:Al">
      <sch:let name="content">
        <xsl:for-each select="self::tekst:Al">
        <xsl:value-of select="."/>
        </xsl:for-each>
      </sch:let>
      <sch:report id="STOP0005" role="error"
        test="normalize-space($content) = '' and not(tekst:InlineTekstAfbeelding)"> {"code": "STOP0005", "element": "<sch:value-of select="ancestor::tekst:*[@eId][1]/local-name()"/>", "eId": "<sch:value-of select="ancestor::tekst:*[@eId][1]/@eId"/>", "melding": "De alinea voor element <sch:value-of select="ancestor::tekst:*[@eId][1]/local-name()"/> met id <sch:value-of select="ancestor::tekst:*[@eId][1]/@eId"/> bevat geen tekst. Verwijder de lege alinea", "ernst": "fout"},</sch:report>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern id="sch_tekst_027" see="tekst:Kop">
    ...
    <sch:rule context="tekst:Kop">
      <sch:let name="content">
       <xsl:for-each select="self::tekst:Kop">
         <xsl:value-of select="."/>
       </xsl:for-each>
      </sch:let>
      <sch:report id="STOP0006" role="error" test="normalize-space($content) = ''"> {"code": "STOP0006", "element": "<sch:value-of select="ancestor::tekst:*[@eId][1]/local-name()"/>", "eId": "<sch:value-of select="ancestor::tekst:*[@eId][1]/@eId"/>", "melding": "De kop voor element <sch:value-of select="ancestor::tekst:*[@eId][1]/local-name()"/> met id <sch:value-of select="ancestor::tekst:*[@eId][1]/@eId"/> bevat geen tekst. Corrigeer de kop of verplaats de inhoud naar een ander element", "ernst": "fout"},</sch:report>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  ...
  <sch:pattern id="sch_tekst_032" see="tekst:Illustratie tekst:InlineTekstAfbeelding">
    ...
    <sch:rule context="tekst:Illustratie | tekst:InlineTekstAfbeelding">
      ...
      <sch:report id="STOP0046" role="warning" test="@kleur"> {"code": "STOP0046", "ouder": "<sch:value-of select="local-name(ancestor::*[@eId][1])"/>", "eId": "<sch:value-of
        select="ancestor::*[@eId][1]/@eId"/>", "melding": "De Illustratie binnen <sch:value-of select="local-name(ancestor::*[@eId][1])"/> met eId <sch:value-of
        select="ancestor::*[@eId][1]/@eId"/> heeft een waarde voor attribuut @kleur. Dit attribuut wordt genegeerd in de publicatie van STOP @@@VERSIE@@@. Verwijder het attribuut @kleur.", "ernst": "waarschuwing"},</sch:report>
    </sch:rule>
  </sch:pattern>
  <!--  -->
  <sch:pattern id="sch_tekst_005" see="tekst:Divisietekst tekst:Kop">
    ...
    <sch:rule
      ...
      <sch:assert id="STOP0041" role="error"
        test="(parent::tekst:Bijlage | parent::tekst:Toelichting | parent::tekst:Motivering | parent::tekst:AlgemeneToelichting) and (count(parent::tekst:*/tekst:Divisietekst) + count(parent::tekst:*/tekst:Divisie) = 1)"
  ...
  <sch:pattern id="sch_tekst_014" see="tekst:table tekst:tgroup">
    ...
    <sch:rule context="tekst:table[tekst:tgroup[@cols]]">
  ...
  <sch:pattern id="sch_tekst_016" see="tekst:table tekst:entry">
    ...
    <sch:rule
      context="tekst:entry[@namest and @colname][ancestor::tekst:tgroup/tekst:colspec[@colname][@colnum]]">
  ...
  <!--  -->
  <sch:pattern>
    <sch:rule context="tekst:entry[@namest][@nameend][ancestor::tekst:tgroup/tekst:colspec[@colname][@colnum]]">
  ...
  <!--  -->
  <sch:pattern>
    <sch:rule context="tekst:entry[@colname][ancestor::tekst:tgroup/tekst:colspec[@colname]]">
  ...
  <sch:pattern id="sch_tekst_017" see="tekst:table">
    ...
    <sch:rule context="tekst:tgroup[tekst:colspec[@colname][@colnum]]/tekst:thead | tekst:tgroup[tekst:colspec[@colname][@colnum]]/tekst:tbody">
