...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   <!--RULE -->
   <xsl:template match="data:informatieobjectRefs" priority="1000" mode="M6">
   ...
   <!--RULE -->
   <xsl:template match="data:rechtsgebieden" priority="1004" mode="M7">
   ...
   <!--RULE -->
   <xsl:template match="data:overheidsdomeinen" priority="1003" mode="M7">
   ...
   <!--RULE -->
   <xsl:template match="data:onderwerpen" priority="1002" mode="M7">
   ...
   <!--RULE -->
   <xsl:template match="data:alternatieveTitels" priority="1001" mode="M7">
   ...
   <!--RULE -->
   <xsl:template match="data:opvolging" priority="1000" mode="M7">
   ...
   <!--RULE -->
   <xsl:template match="data:alternatieveTitel" priority="1000" mode="M8">
   ...
   <!--RULE -->
   <xsl:template match="data:opvolgerVan" priority="1000" mode="M9">
      ...
      <xsl:choose>
        ...
        {"code": "STOP1024", "substring": "<xsl:text/>
            ...
            <xsl:value-of select="./string()"/>
            <xsl:text/>) wordt niet verwezen naar een expression van een Regeling of Informatieobject ('/AKN/act/...' of '/join/id/regdata/...'). Corrigeer de verwijzing opvolgerVan.", "ernst": "fout"},<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M9"/>
   </xsl:template>
   ...
   <!--RULE -->
   <xsl:template match="data:TekstReferentie" priority="1000" mode="M10">
   ...
   <!--PATTERN sch_data_012datum ondertekening verplicht voor off pub van besluit en verboden voor off pub van
      kennisgeving-->
   <!--RULE -->
   <xsl:template match="data:OfficielePublicatieMetadata[data:soortPublicatie = '/join/id/stop/soortpublicatie_001']"
                 priority="1001"
                 mode="M11">

		<!--ASSERT fout-->
      <xsl:choose>
         <xsl:when test="data:ondertekendOp"/>
         <xsl:otherwise> {"code": "STOP1032", "melding": "De officiële publicatie van het besluit heeft geen datum ondertekening.", "ernst": "fout"},<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M11"/>
   </xsl:template>
   <!--RULE -->
   <xsl:template match="data:OfficielePublicatieMetadata[data:soortPublicatie = '/join/id/stop/soortpublicatie_002']"
                 priority="1000"
                 mode="M11">

		<!--REPORT fout-->
      <xsl:if test="data:ondertekendOp"> {"code": "STOP1033", "melding": "De officiële publicatie van een besluit heeft ten onrechte een datum ondertekening.", "ernst": "fout"},<xsl:value-of select="string('&#xA;')"/>
      </xsl:if>
      <xsl:apply-templates select="*" mode="M11"/>
   </xsl:template>
   ...
   <xsl:template match="*:heeftGeboorteregeling" priority="1000" mode="M14">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
        {"code": "STOP1060", "Expressie": "<xsl:text/>
            <xsl:value-of select="."/>
            <xsl:text/>", "melding": "heeftGeboorteregeling verwijst naar <xsl:text/>
            <xsl:value-of select="."/>
            <xsl:text/>. Dit is geen expression van een Regeling ('/AKN/nl/act/...'). Corrigeer de verwijzing heeftGeboorteregeling.", "ernst": "fout"},<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M14"/>
   </xsl:template>
