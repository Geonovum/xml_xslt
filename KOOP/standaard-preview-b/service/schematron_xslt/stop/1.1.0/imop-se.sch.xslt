...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   <xsl:template match="se:PointSymbolizer" priority="1000" mode="M16">
      ...
      <!--ASSERT fout-->
      <xsl:choose>
         <xsl:when test="./se:Graphic/se:Mark/se:Fill/se:SvgParameter"/>
         <xsl:otherwise>
        {"code": "STOP3138", "ID": "<xsl:text/>
            <xsl:value-of select="./se:Name"/>
            <xsl:text/>", "melding": "De PointSymbolizer van Rule:Name <xsl:text/>
            <xsl:value-of select="./se:Name"/>
            <xsl:text/> heeft niet de vorm se:Graphic/se:Mark/se:Fill/se:GraphicFill/se:SvgParameter, dit is verplicht. Wijzig deze symbolizer.", "ernst": "fout"},<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
