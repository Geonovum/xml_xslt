...
<xsl:stylesheet xmlns:my="functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xlink="http://www.w3.org/1999/xlink" version="2.0" xmlns:ow-dc="http://www.geostandaarden.nl/imow/bestanden/deelbestand" xmlns:ow="http://www.geostandaarden.nl/imow/owobject" xmlns:sl="http://www.geostandaarden.nl/bestanden-ow/standlevering-generiek" xmlns:l="http://www.geostandaarden.nl/imow/locatie" xmlns:rg="http://www.geostandaarden.nl/imow/regelingsgebied" xmlns:r="http://www.geostandaarden.nl/imow/regels" xmlns:rol="http://www.geostandaarden.nl/imow/regelsoplocatie">
   ...
   <xsl:param name="base.dir" select="string('file:/C:/Werkbestanden/Geonovum/KOOP/Bruidsschat/1.0.4/temp')"/>
   <xsl:param name="waardelijsten" select="document('waardelijsten OW 2.1.0.xml')//waardelijst"/>
   ...
   <xsl:variable name="consolidatie" select="fn:collection(concat($base.dir,'?select=*.xml'))//RegelingCompact//(Artikel|Lid)/@wId"/>
   ...
   <xsl:variable name="naamBevoegdGezag" select="$OIN/item[fn:ends-with(BG,$idBevoegdGezag)]/naam"/>
   ...
   <xsl:template match="/">
      ...
      <xsl:result-document encoding="Windows-1252" href="../opdracht/log.txt" method="text">
         <!-- controle op mapping tussen wId's in consolidaties en wIdJurCon.xml -->
         <xsl:variable name="check">
            <xsl:for-each select="$wId2JuriconnectId//(artikel,lid)">
               <xsl:variable name="id" select="@wId"/>
               <xsl:if test="not($consolidatie[string(.) eq $id])">
                  <xsl:element name="item">
                     <xsl:value-of select="$id"/>
                  </xsl:element>
               </xsl:if>
            </xsl:for-each>
            <xsl:for-each select="$consolidatie">
               <xsl:variable name="id" select="string(.)"/>
               <xsl:if test="not($wId2JuriconnectId//(artikel,lid)[@wId eq $id])">
                  <xsl:element name="item">
                     <xsl:value-of select="$id"/>
                  </xsl:element>
               </xsl:if>
         ...
         <xsl:value-of select="concat('[Controle op mapping tussen wId''s in consolidatie en wIdJurCon.xml]','&#10;')"/>
         <xsl:choose>
            <xsl:when test="$check/item">
               <xsl:value-of select="concat('Er zijn ',count($check/item),' fouten gevonden in de mapping tussen wId in consolidatie en wIdJurCon.xml:','&#10;')"/>
               <xsl:for-each select="$check/item">
                  <xsl:value-of select="concat(position(),': wId ''',.,''' komt niet voor in zowel de consolidatie als in wIdJurCon.xml.','&#10;')"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="concat('Er zijn geen fouten gevonden in de mapping tussen wId in consolidatie en wIdJurCon.xml.','&#10;')"/>
            </xsl:otherwise>
         </xsl:choose>
         <xsl:value-of select="string('&#10;')"/>
         <!-- controle op mapping tussen wId's en juriconnectId's -->
         <xsl:variable name="check">
         ...
         <xsl:value-of select="concat('[Controle op mapping tussen wId''s en juriconnectId''s]','&#10;')"/>
         <xsl:choose>
            <xsl:when test="$check/item">
               <xsl:value-of select="concat('Er zijn ',count($check/item),' fouten gevonden in de mapping tussen wId in owRegeltekst.xml en juriconnectId:','&#10;')"/>
               <xsl:for-each select="$check/item">
                  <xsl:value-of select="concat(position(),': juriconnectId ''',.,''' ontbreekt in de mapping.','&#10;')"/>
               </xsl:for-each>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="concat('Er zijn geen fouten gevonden in de mapping tussen wId in owRegeltekst.xml en juriconnectId.','&#10;')"/>
            </xsl:otherwise>
         </xsl:choose>
         <xsl:value-of select="string('&#10;')"/>
         ...
         <xsl:variable name="check">
            <xsl:for-each select="$owActiviteit">
               ...
               <xsl:if test="not($waardelijsten[term='Activiteitengroep']/waarden/waarde[uri=$id])">
                  <xsl:element name="item">
      ...
      <xsl:result-document encoding="UTF-8" href="owRegeltekst.xml" indent="yes" method="xml" version="1.0">
         <xsl:element name="ow-dc:owBestand">
            ...
            <xsl:attribute name="xsi:schemaLocation" select="string('http://www.geostandaarden.nl/imow/bestanden/deelbestand https://register.geostandaarden.nl/xmlschema/tpod/v2.0.0/bestanden-ow/deelbestand-ow/IMOW_Deelbestand.xsd')"/>
                        ...
                        <xsl:element name="r:RegelVoorIedereen">
                           ...
                           <xsl:element name="r:locatieaanduiding">
                              <xsl:element name="l:LocatieRef">
                                 <xsl:attribute name="xlink:href" select="fn:string-join(('nl.imow',fn:string-join(($idBevoegdGezag,'ambtsgebied','9999999999'),'.')),'-')"/>
                              </xsl:element>
                           </xsl:element>
                           <xsl:for-each select="$owRegeltekst[@juriconnectId=$id/@juriconnectId]/Annotatie">
                              <xsl:choose>
                     ...
                     <xsl:element name="ow-dc:owObject">
                        <xsl:element name="rol:Activiteit">
                           ...
                           <xsl:element name="rol:groep">
                              <xsl:value-of select="($waardelijsten[term='Activiteitengroep']/waarden/waarde[uri=current()/rol:groep]/uri,'http://standaarden.omgevingswet.overheid.nl/activiteit/id/concept/Onbekend')[1]"/>
                           </xsl:element>
         ...
         <xsl:element name="ow-dc:owBestand">
            ...
            <xsl:element name="sl:standBestand">
               ...
               <xsl:element name="sl:inhoud">
                  ...
                  <xsl:element name="sl:objectTypen">
                     <xsl:for-each select="('Ambtsgebied')">
                        <xsl:element name="sl:objectType">
                           <xsl:value-of select="."/>
                        </xsl:element>
                     </xsl:for-each>
                  ...
                  <xsl:element name="ow-dc:owObject">
                     <xsl:element name="l:Ambtsgebied">
                        <xsl:element name="l:identificatie">
                           <xsl:value-of select="fn:string-join(('nl.imow',fn:string-join(($idBevoegdGezag,'ambtsgebied','9999999999'),'.')),'-')"/>
                        </xsl:element>
                        <xsl:element name="l:noemer">
                           <xsl:value-of select="$naamBevoegdGezag"/>
                        </xsl:element>
                        <xsl:element name="l:bestuurlijkeGrenzenVerwijzing">
                           <xsl:element name="l:BestuurlijkeGrenzenVerwijzing">
                              <xsl:element name="l:bestuurlijkeGrenzenID">
                                 <xsl:value-of select="fn:upper-case($idBevoegdGezag)"/>
                              </xsl:element>
                              <xsl:element name="l:domein">
                                 <xsl:value-of select="string('NL.BI.BestuurlijkGebied')"/>
                              </xsl:element>
                              <xsl:element name="l:geldigOp">
                                 <xsl:value-of select="string('2022-05-14')"/>
                              </xsl:element>
                           </xsl:element>
                        </xsl:element>
                     </xsl:element>
      ...
      <xsl:element name="Aanleveringen" namespace="{$namespace}">
         <xsl:attribute name="xsi:schemaLocation" select="string('http://www.geostandaarden.nl/bestanden-ow/manifest-ow https://register.geostandaarden.nl/xmlschema/tpod/v2.0.0/bestanden-ow/generiek/manifest-ow.xsd')"/>
