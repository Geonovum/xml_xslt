...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:basisgeo="http://www.geostandaarden.nl/basisgeometrie/1.0" xmlns:geo="https://standaarden.overheid.nl/stop/imop/geo/" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:ow-dc="http://www.geostandaarden.nl/imow/bestanden/deelbestand" xmlns:sl="http://www.geostandaarden.nl/bestanden-ow/standlevering-generiek" xmlns:l="http://www.geostandaarden.nl/imow/locatie" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:data="https://standaarden.overheid.nl/stop/imop/data/" xmlns:lvbba="https://standaarden.overheid.nl/lvbb/stop/aanlevering/" version="2.0">
   ...
   <xsl:template match="/">
      <xsl:for-each select="$bg-unique">
         <xsl:variable name="gio-eindverantwoordelijke-id" select="fn:tokenize(.,'/')[last()]"/>
         <xsl:variable name="gio-eindverantwoordelijke-naam" select="$OIN/item[fn:ends-with(BG,$gio-eindverantwoordelijke-id)]/naam"/>
         <xsl:variable name="gio-leveringsid" select="fn:string-join(($gio-eindverantwoordelijke-id,fn:format-date(fn:current-date(),'[Y0001]-[M01]-[D01]')),'_')"/>
         <xsl:variable name="gml" select="collection(concat($input.dir,'?select=*.gml;recurse=yes'))//geo:GeoInformatieObjectVersie[fn:tokenize(geo:FRBRWork,'/')[5]=$gio-eindverantwoordelijke-id]"/>
                        ...
                        <xsl:attribute name="gio-id">
                           <xsl:choose>
                              <xsl:when test="contains(fn:tokenize(current(),'/')[7],'ib_aangewezen')">
                                 <xsl:value-of select="concat('nl.imow-',$gio-eindverantwoordelijke-id,'.gebiedengroep.aangewOppervlaktewaterlichamen')"/>
                              </xsl:when>
                              <xsl:when test="contains(fn:tokenize(current(),'/')[7],'ib_niet_aangewezen')">
                                 <xsl:value-of select="concat('nl.imow-',$gio-eindverantwoordelijke-id,'.gebiedengroep.nietaangewOppvlaktewaterlichamen')"/>
                              </xsl:when>
                              <xsl:otherwise>
                                 <xsl:value-of select="concat('nl.imow-',$gio-eindverantwoordelijke-id,'.gebiedengroep.onbekend')"/>
                              </xsl:otherwise>
                           </xsl:choose>
                        </xsl:attribute>
         ...
         <xsl:result-document href="{fn:string-join(($gio-eindverantwoordelijke-id,concat($gio-eindverantwoordelijke-id,'Locaties.xml')),'/')}" method="xml" indent="yes">
            <xsl:element name="ow-dc:owBestand">
               ...
               <xsl:element name="sl:standBestand">
                  <xsl:element name="sl:dataset">
                     <xsl:value-of select="$gio-eindverantwoordelijke-id"/>
                  </xsl:element>
                              ...
                              <xsl:element name="l:groepselement">
                                 <xsl:for-each select="fn:subsequence(current-group(),2)">
                                    <xsl:element name="l:GebiedRef">
                                       <xsl:attribute name="xlink:href" select="concat('nl.imow-',$gio-eindverantwoordelijke-id,'.gebied.2020',fn:format-number(@index,'000000'))"/>
                                    </xsl:element>
                                 </xsl:for-each>
                              </xsl:element>
                           </xsl:element>
                     ...
                     <xsl:element name="sl:stand">
                        <xsl:element name="ow-dc:owObject">
                           <xsl:element name="l:Gebied">
                              <xsl:element name="l:identificatie">
                                 <xsl:value-of select="concat('nl.imow-',$gio-eindverantwoordelijke-id,'.gebied.2020',fn:format-number(current()/@index,'000000'))"/>
                              </xsl:element>
         ...
         <xsl:for-each select="$locaties/locatie">
            <xsl:result-document href="{fn:string-join(($gio-eindverantwoordelijke-id,concat(current()/@id,'.gml')),'/')}" method="xml" indent="yes">
         ...
         <xsl:value-of select="concat('[BG ',$gio-eindverantwoordelijke-id,']','&#10;')"/>
