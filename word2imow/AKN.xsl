...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
    ...
    <!--xsl:param name="wId_bg" select="string('mnre1045')"/>
    ...
    <xsl:template match="element()" mode="eId">
        <xsl:variable name="index" select="fn:index-of($element,name())"/>
        <xsl:variable name="count" select="if ($element_wId_eId[$index] eq 'onwaar') then fn:string-join(('o',string(count(.|preceding-sibling::*[name() eq $element[$index]]))),'_') else null"/>
        <xsl:choose>
