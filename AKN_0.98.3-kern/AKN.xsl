...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns="https://standaarden.overheid.nl/stop/imop/tekst/" xpath-default-namespace="https://standaarden.overheid.nl/stop/imop/tekst/">
    ...
    <xsl:param name="element" select="('Aanhef','Afdeling','AlgemeneToelichting','Artikel','ArtikelgewijzeToelichting','Begrip','Begrippenlijst','Bijlage','Boek','Citaat','Deel','Divisie','Deel','ExtIoRef','Figuur','FormeleDivisie','FormeleInhoud','Hoofdstuk','Inhoudsopgave','IntIoRef','Li','Lichaam','Lid','Lijst','Motivering','Nawerk','RegelingOpschrift','Paragraaf','Sluiting','Subparagraaf','Subsubparagraaf','table','Titel','Toelichting','WijzigArtikel','WijzigBijlage')"/>
    <xsl:param name="element_ref" select="('formula_1','subchp','genrecital','art','artrecital','item','list','cmp','book','cit','part','div','part','ref','img','fdvs','fcontent','chp','toc','ref','item','body','para','list','acc','app','longTitle','subsec','formula_2','subsec','subsec','table','title','recital','art','cmp')"/>
    <xsl:param name="element_wId_eId" select="('waar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','waar','onwaar','onwaar','waar','onwaar','onwaar','onwaar','waar','waar','onwaar','waar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar','onwaar')"/>

    <!-- Vul hieronder de identifier voor het bevoegd gezag en het versienummer in. -->

    <xsl:param name="wId_bg" select="string('gm0037')"/>
    ...
    <xsl:template match="element()" mode="eId">
        ...
        <xsl:variable name="count" select="if ($element_ref[$index] ne 'body') then fn:string-join(('o',count(.|preceding-sibling::*[local-name() eq $element[$index]])),'_') else null"/>
        <xsl:choose>
